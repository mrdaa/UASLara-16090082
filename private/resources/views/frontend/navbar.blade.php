    ...
    <div class="wrap_menu">
        <nav class="menu">
            <ul class="main_menu">
                ...
                @php($cats = DB::table('categories')->select('nama_kategori')->where('deleted',0)->distinct()->get())

                @foreach ($cats as $item)
                <li>
                <a href="#">{{ ucfirst(trans($item->nama_kategori)) }}</a>
                    <ul class="sub_menu">
                        @php($subcats = DB::table('categories')->select('sub_kategori')->where('nama_kategori','=',$item->nama_kategori)->where('deleted',0)
                        ->get())
